...
import { authErrorStore } from '../store/authErrorStore';
import TokenService from '../services/tokenService';
import { PUBLIC_ENDPOINTS, SECURE_STORAGE_KEYS } from '../constants/security';
import type { UserInfo } from '../types/auth';
import { getAuthErrorMessage } from '../types/enums/AuthErrorEnum';
// 创建axios实例
...
apiClient.interceptors.response.use(
  ...
  async (error) => {
    ...
    if (error.response?.status === 401) {
      ...
      const refreshTokenEndpoint = isAdminPath
        ? API_ENDPOINTS.ADMIN.AUTH.REFRESH_TOKEN
        : API_ENDPOINTS.COMMON.AUTH.TOKEN;
      // 检查是否是刷新令牌请求本身失败
      if (originalRequest.url === refreshTokenEndpoint) {
        ...
        // 显示友好的错误信息
        const friendlyErrorMessage = getAuthErrorMessage(errorMessage);
        if (!isCurrentPagePublic) {
          // 非公开页面，跳转到登录页面
          authErrorStore.getState().showError(friendlyErrorMessage);
          // 3秒后跳转到相应的登录页面
          setTimeout(() => {
          ...
          // 公开页面，只显示错误信息，不跳转
          authErrorStore.getState().showError(friendlyErrorMessage, { redirectToLogin: false });
        }
        // 终止Promise链，避免无限循环
        ...
        if (refreshToken) {
          try {
            ...
            if (response.code === 200) {
              ...
              // 处理刷新令牌失败的情况
              const friendlyErrorMessage = getAuthErrorMessage(response.message);
              authErrorStore.getState().showError(friendlyErrorMessage);
              // 清理用户状态
              if (isAdminPath) {
                adminStore.getState().logout();
            ...
            const friendlyErrorMessage =
              refreshError instanceof Error ? getAuthErrorMessage(refreshError.message) : '登录已过期，请重新登录';
            authErrorStore.getState().showError(friendlyErrorMessage);
            setTimeout(() => {
              window.location.href = isAdminPath ? ROUTES.ADMIN_LOGIN : ROUTES.LOGIN;
            }, 3000);
