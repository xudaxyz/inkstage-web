...
const CommentItem: React.FC<CommentItemProps> = ({
  ...
  const fetchReplies = useCallback(
    async (pageNum: number = 1, isRefresh: boolean = false): Promise<void> => {
  ...
  const topReplies = useMemo(() => {
    ...
    return [...comment.replies].sort((a, b) => b.likeCount - a.likeCount).slice(0, DEFAULT_VISIBLE_REPLIES);
  }, [comment.replies]);
  // 分页后的子评论
    ...
    <div key={comment.id} className="border-b border-gray-50 dark:border-b dark:border-gray-700 pb-6 group">
      ...
      <div className="flex items-center gap-2 mb-2">
        ...
        {comment.userId ? (
          ...
          <span className="font-bold text-[#373A40] dark:text-gray-200 ml-1">{comment.nickname}</span>
        )}
        {comment.top === CommentTopStatus.TOP && (
          <span className="inline-flex items-end text-xs bg-red-100 text-red-600 px-2 py-1 rounded">置顶</span>
        )}
      </div>

      {/* 评论内容 */}
      <div className="ml-12 mb-3 text-gray-700 dark:text-gray-300 font-medium text-base leading-relaxed">
        {displayContent}
        {isLongComment && (
          <button onClick={toggleExpand} className="text-blue-600 text-sm ml-2">
            {isExpanded ? '收起' : '展开'}
          </button>
        )}
      </div>
      ...
      <div className="ml-12 flex items-center gap-4 text-sm text-gray-500 relative">
        <span className="flex items-center gap-1">{comment.createTime ? getRelativeTime(comment.createTime) : ''}</span>
        <button
          className={`flex items-center min-w-7 ${comment.isLiked ? 'text-red-500' : 'text-gray-500'}`}
          onClick={() => toggleLike(comment.id)}
        >
          <LikeOutlined />
          <span className="min-h-1 ml-1">{comment.likeCount > 0 ? comment.likeCount : ''}</span>
        </button>
        <button
        ...
        >
          <DislikeOutlined />
          <span className="min-h-1 ml-1">{comment.dislikeCount > 0 ? comment.dislikeCount : ''}</span>
        </button>
        <button
            ...
            <div className="mt-3 flex flex-wrap items-center justify-start">
              ...
              {isRepliesExpanded && needPagination && (
                <div className="ml-6 flex items-center gap-2">
                  <span className="text-sm text-gray-500">共 {Math.ceil(totalReplies / REPLIES_PAGE_SIZE)} 页</span>
                  <Pagination
