...
import { Button, Card, Dropdown, Form, Input, message, Modal, Select, Space, Table, Tag, Typography } from 'antd';
import {
  CalendarOutlined,
  CheckCircleOutlined,
  CloseCircleOutlined,
...
// 置顶状态选项
const topOptions = [
  { value: AllowTopEnum.TOP, label: '置顶' },
  { value: AllowTopEnum.NOT_TOP, label: '不置顶' }
];
const AdminComments: React.FC = () => {
  ...
  const fetchComments = useCallback(
    async (pageNum: number = 1, pageSize: number = 10): Promise<void> => {
      setLoading(true);
      try {
  ...
  const handleUpdateTop = async (id: number, shouldTop: boolean): Promise<void> => {
    try {
      ...
      if (response.code === 200) {
        message.success(response.message || (shouldTop ? '评论置顶成功' : '取消评论置顶成功'));
        await fetchComments(pagination.pageNum, pagination.pageSize);
      } else {
        message.error(response.message || '更新评论置顶状态失败');
      }
    } catch (error) {
  ...
  const handleRejectComment = async (): Promise<void> => {
    ...
    try {
      const response = await commentService.admin.updateCommentStatus(
        currentReviewCommentId,
    ...
    form
      ...
      .then(async (values) => {
        if (isEditing && currentComment) {
          // 更新评论相关
          const response = await commentService.admin.updateComment(
            currentComment.id,
            values.content,
            values.top,
            values.reviewStatus,
          ...
          if (response.code === 200) {
            ...
            message.error(response.message || '评论更新失败');
          }
        }
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (reviewStatus: CommentStatusEnum): React.ReactNode => (
        <Tag color={getStatusColor(reviewStatus)}>{CommentStatusMap[reviewStatus] || reviewStatus}</Tag>
      )
    },
    {
      title: '置顶',
    ...
    },
    {
      ...
      render: (_: unknown, record: AdminArticleCommentList): React.ReactNode => (
        <Space size="middle">
          ...
          >
            <Button variant={'filled'} color={'orange'} icon={<CheckCircleOutlined />} className="text-orange-500">
              审核
            </Button>
          </Dropdown>
          <Dropdown
            menu={{
              items: [
                ...
                },
                {
                  key: 'top',
                  label: (
                    <span onClick={() => handleUpdateTop(record.id, record.top !== CommentTopStatus.TOP)}>
          ...
          >
            <Button variant={'filled'} color={'default'} icon={<DownOutlined />}>
              更多
            </Button>
          </Dropdown>
        </Space>
