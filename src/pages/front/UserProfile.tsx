...
const UserProfile: React.FC = () => {
    ...
    const fetchUserProfile = async (): Promise<void> => {
      if (id) {
        try {
          ...
          if (!paramNickname) {
            navigate(ROUTES.USER_PROFILE(formattedUser.id), { replace: true });
          }
        } catch {
  ...
  const articlesFetcher = useCallback(
    async (pageNum: number, pageSize: number): Promise<ApiPageResponse<IndexArticleList>> => {
      ...
      const response = await articleService.getUserArticles(id, pageNum, pageSize);
        ...
        <div className="flex-1">
          <h3 className="text-xl hover:text-cyan-600 text-gray-800 dark:text-gray-300 font-semibold mb-3 leading-tight">
            <a
              ...
              className="transition-colors duration-300 group-hover:translate-x-1"
            >
              {article.title}
            </a>
          </h3>
