      ...
      style={{
        ...
        backgroundColor:
          notification.readStatus === ReadStatus.UNREAD
            ? `${isDarkMode ? '#4a5565' : '#e5e5e5'}`
            : `${isDarkMode ? '#1e2939' : 'white'}`
      }}
      styles={{
        body: {
          padding: '12px 20px',
          ...
          <div className="flex items-center justify-end space-x-2">
            {notification.readStatus === ReadStatus.UNREAD && (
              <>
                {' '}
                {isDarkMode ? (
                  <Button type="text" icon={<CheckOutlined />} onClick={() => markAsRead(notification.id)}>
                    标为已读
                  </Button>
                ) : (
            ...
            <Button type="link" danger icon={<DeleteOutlined />} onClick={() => deleteNotification(notification.id)}>
              删除
            </Button>
          </div>
        </div>
