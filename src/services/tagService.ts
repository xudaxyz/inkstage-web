 ...
 * @returns 标签列表
 */
export const getAllTags = async (): Promise<ApiResponse<FrontTag[]>> => {
 ...
 * @returns 激活状态的标签列表
 */
export const getActiveTags = async (): Promise<ApiResponse<FrontTag[]>> => {
...
const admin = {
   ...
   * @returns 分页结果
   */
  getTagsByPage: async (keyword: string, pageNum: number, pageSize: number): Promise<ApiResponse<TagPageResponse>> => {
   ...
   * @returns 添加后的标签信息
   */
  addTag: async (tag: TagCreateUpdateParams): Promise<ApiResponse<AdminTag>> => {
